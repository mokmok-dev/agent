...
|---|---|---|
| セッション横断の永続層 | `~/.local/share/opencode/opencode.db` 1本を全セッションで共有 | 1本のグローバルイベントストア (`/tmp/mokmokd-events.db` デフォルト) |
| セッション固有データ | `storage/session_diff/ses_<id>.json` に1セッション1ファイル | (設計のみ) `docs/session-store.md` のセッションストア案 |
| セッション状態の保持 | SQLite は保持しない(差分は JSON) | SQLite は保持しない(append-only 監査ログ) |

→ 「共有 SQLite = セッション横断の永続層」「セッション固有データ = JSON ファイル」という分担が答え。
...
## agentd の共有 SQLite(グローバルイベントストア)

- 実装は `agentd/src/eventstore.rs`。1つの SQLite DB をデーモン全体・全セッションで共有する、イベントバス用の append-only 永続化層として動作する。
- 中身は `events` テーブル1つの append-only ログ(`seq`・`id`・`source`・`specversion`・`type`・`time`・`event` の各列、`event` 列に CloudEvents 1.0 エンベロープ JSON を逐語保存)。キャッシュ・インデックス・キーバリュー(設定)テーブルは一切ない。
- スキーマは `PRAGMA user_version` によるバージョン付き forward-only マイグレーションで管理される。
- DB ファイルはデフォルトで `/tmp/mokmokd-events.db`、`--db-path` で変更可能(`agentd/src/main.rs` の `Serve` サブコマンド定義)。open() 時に親ディレクトリが自動作成される。

## 並行性・単一ライタ保証・耐久性

- SQLite コネクションは単一の専用ライタスレッド(スレッド名 `"eventstore"`)だけが所有する。async のコレクタタスクは bounded mpsc チャネル(容量 1024)でイベントを送り、ライタが1トランザクションずつバッチ INSERT する構成で、async ランタイムから SQLite に直接触る経路はない。
- 単一ライタ構造は Unix ソケット排他によるデーモンの単一インスタンス制約でも構造的に保証される。既存ソケットが生きていれば `AlreadyRunning` エラーを返して起動しない(`agentd/src/server.rs`)。
- ロック・耐久性設定は `journal_mode=WAL` と `synchronous=NORMAL`。マイグレーションは各 pending を独自トランザクションで適用して `user_version` を更新し、DB スキーマが新しい場合は `UnsupportedSchema` で拒否する。
- agentd のイベントストアはグローバル監査ログであり、セッション状態は保持しない。

## 役割の違い

2つの保存層は役割が完全に分かれる:

1. **共有 SQLite** はセッションを横断する単一の永続層。
   - opencode 本体: 全セッション共通の共有 DB。
   - agentd: 全セッションのイベントを時系列で残す append-only の監査ログ。
...
- ローカル検証時に `docs/session-store.md` が存在した(セッション開始時の untracked ファイル)が、現時点のリポジトリには存在しない。検証エージェント間でこのファイルの有無に関して矛盾した観測が残っており、「セッションストアの設計文書」の扱いは要確認。本リポジトリのソースコードにはセッション概念の実装は存在しない(グローバルイベントストアのみ)。
