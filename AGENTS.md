...
### 7. Subagent Usage (Context Window Efficiency)

- You MUST actively use subagents (Task tool) to execute work efficiently while conserving the parent agent's context window.
- You MUST delegate exploration (file searches, broad code reading, multi-step research) to subagents instead of reading many files inline.
- You SHOULD run independent subagent tasks in parallel whenever possible.
- Subagent prompts MUST be self-contained: include all necessary context and specify exactly what to report back.
- Subagents MUST only do research/exploration unless explicitly told otherwise; the parent agent performs the actual edits.
- After a subagent returns, you MUST rely on its summary and NOT re-read files it already digested, unless verification requires it.
